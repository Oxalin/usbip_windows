...
void try_save_config(PPDO_DEVICE_DATA pdodata, struct _URB_CONTROL_DESCRIPTOR_REQUEST *req, int in_len)
{
  ...
  pdodata->dev_config = ExAllocatePoolWithTag(NonPagedPool, in_len,
                                              BUSENUM_POOL_TAG);
...
static void copy_iso_data(char *dest, ULONG dest_len, char *src,
  ULONG src_len, struct _URB_ISOCH_TRANSFER *urb) {
    ULONG i;
    ULONG offset;
    offset = 0;
...
//    KdPrint(("ISO Packet: [%d] len: %d stat: %d off: %d\n",i,urb->IsoPacket[i].Length,urb->IsoPacket[i].Status,urb->IsoPacket[i].Offset));
        ...
        RtlCopyMemory(dest + urb->IsoPacket[i].Offset, src + offset,
                      urb->IsoPacket[i].Length);
        offset += urb->IsoPacket[i].Length;
    }

    if (offset != src_len)
        KdPrint(("Warning, why offset:%d != src_len:%d", offset, src_len));
...
int process_write_irp(PPDO_DEVICE_DATA pdodata, PIRP irp) {
    ...
    for (le = pdodata->ioctl_q.Flink; le != &pdodata->ioctl_q;
         le = le->Flink) {
...
//      return STATUS_INVALID_PARAMETER;
    ...
    if (type == USB_ENDPOINT_TYPE_ISOCHRONOUS) {
        if (h->u.ret_submit.number_of_packets != urb->NumberOfPackets) {
  ...
  //      KdPrint(("ISO: %d %d %d %d %d\n",  i,  ip_desc->offset, ip_desc->length, ip_desc->actual_length, ip_desc->status ));
            if (ip_desc->offset > urb->IsoPacket[i].Offset) {
                KdPrint(("Warning, why offset changed?%d %d %d %d\n",
                          i, ip_desc->offset, ip_desc->actual_length,
                          urb->IsoPacket[i].Offset));
                goto end;
            }

            urb->IsoPacket[i].Length = ip_desc->actual_length;
            urb->IsoPacket[i].Status = tran_usb_status(ip_desc->status,
              in_len, USB_ENDPOINT_TYPE_ISOCHRONOUS);
            ip_desc++;
        }
    ...
    if (in_len) {
        ...
        if (urb->TransferBuffer)
            ...
            buf = MmGetSystemAddressForMdlSafe(urb->TransferBufferMDL,
              NormalPagePriority);
        }
        else
    ...
    /* it seems Windows client USB driver will think IoCompleteRequest is
     * running at DISPATCH_LEVEL so without this it will change IRQL
     * sometimes, and introduce to a dead of my userspace program */
                               ...
                               unsigned int *offset, unsigned char type)
{
    ...
    if (offsetTemp >= config->wTotalLength)
        return NULL;

    do {
        if (offsetTemp + sizeof(*desc) > config->wTotalLength)
            return NULL;

        desc = (PUSB_COMMON_DESCRIPTOR)((char *)config + offsetTemp);
        if (desc->bLength + offsetTemp > config->wTotalLength)
            return NULL;

        offsetTemp += desc->bLength;
              ...
              PUSB_ENDPOINT_DESCRIPTOR ep_desc,  unsigned char speed)
{
    ...
    pipe->PipeHandle = make_pipe(ep_desc->bEndpointAddress, pipe->PipeType,
                          ep_desc->bInterval);
}
...
int proc_select_config(PPDO_DEVICE_DATA pdodata,
  struct _URB_SELECT_CONFIGURATION * req) {
    ...
    if (!RtlEqualMemory(pdodata->dev_config, req->ConfigurationDescriptor,
    ...
    for (i = 0; i < req->ConfigurationDescriptor->bNumInterfaces; i++) {
        ...
        if (intf->NumberOfPipes > 0) {
            if ((char *)intf + sizeof(*intf) +
              (intf->NumberOfPipes-1)*sizeof(intf->Pipes[0]) - (char *)req
              > req->Hdr.Length) {
        ...
        intf = (USBD_INTERFACE_INFORMATION *)((char *)intf + sizeof(*intf) +
...
NTSTATUS Bus_IoCtl(__in PDEVICE_OBJECT DeviceObject, __in PIRP Irp) {
    ...
    case IOCTL_USBVBUS_PLUGIN_HARDWARE:
        if (sizeof(ioctl_usbvbus_plugin) == inlen ) {
            ...
            status = bus_plugin_dev((ioctl_usbvbus_plugin *) buffer,
                        fdoData, irpStack->FileObject);
        }
        break;
    ...
    case IOCTL_USBVBUS_GET_PORTS_STATUS:
        if (sizeof(ioctl_usbvbus_get_ports_status) == outlen ) {
            ...
            status = bus_get_ports_status((ioctl_usbvbus_get_ports_status *) buffer,
                        fdoData, &info);
        }
        break;
...
END:
    Irp->IoStatus.Status = status;
    // FIXME: it seems to be the cause of bug:
    // https://sourceforge.net/p/usbip/discussion/418507/thread/a09fa85b/?limit=25&page=1#7f98
    // Shouldn't we be rising and lowering IRQL? We are doing the same
    // elsewhere, see at the end of process_write_irp() for same effect and
    // cause.
    IoCompleteRequest(Irp, IO_NO_INCREMENT);
    Bus_DecIoCount(fdoData);
