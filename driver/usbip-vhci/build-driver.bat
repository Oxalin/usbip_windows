...
IF "%BASEDIR%" NEQ "C:\WinDDK\7.1\7600.16385.1" (
set BASEDIR=C:\WinDDK\7.1\7600.16385.1
CALL %BASEDIR%\bin\setenv.bat %BASEDIR% %TYPE%     WLH
cd /d %OCD%
)

cmd /C "set DDKBUILDENV=&& %BASEDIR%\bin\setenv.bat %BASEDIR% %TYPE%     WLH && cd /d %OCD% && build /w"
cmd /C "set DDKBUILDENV=&& %BASEDIR%\bin\setenv.bat %BASEDIR% %TYPE% x64 WLH && cd /d %OCD% && build /w"
